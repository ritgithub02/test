...
w1e,w2e,w3e,w4e,w5e=st.columns(5)
display_image_from_url("https://raw.githubusercontent.com/ritgithub02/data/main/Indian_Institute_of_Technology_(Indian_School_of_Mines)%2C_Dhanbad_Logo.png",w3e.sidebar)
# ---------------------------------------------------------------------------------LOADING-------
...
with t1:
    ...
    with st.sidebar.expander('About us'):
        ...
        # profile1 = "https://www.linkedin.com/in/harsh-prajapati-682b63287/"
        # markdown_text = create_linkedIn_profile_link(profile1, lin)
        # lg01.markdown(markdown_text, unsafe_allow_html=True)



        # profile4 = "22mc0071@iitism.ac.in"
        # markdown_text = create_gmail_profile_link(profile4,gml)
        # lg02.markdown(markdown_text, unsafe_allow_html=True)
    ...
    with t2:
        ...
        with ex:
            ...
            if bit_size_curve_availibility_question == 'Yes':
                ...
                selected_column_BS = a2.selectbox('Choose the Bit Size column', well_df.columns, index=well_df.columns.get_loc('BS') if 'BS' in well_df.columns else 1)
                column_choice_BS = selected_column_BS
                
    
                # a3.write(f"Selected column: {selected_column_BS}.")
            
                # Working as the dropdown menu for selecting the Caliper curve
                column_choice_CALI = a2.selectbox("Choose the Caliper column:", well_df.columns)
                ...
                column_choice_CALI = a2.selectbox('Choose the Caliper column:', well_df.columns, index=well_df.columns.get_loc('CALI') if 'CALI' in well_df.columns else 1)
    
                selected_column_CALI = column_choice_CALI
                # st.write(f"You selected column: {selected_column_CALI} for Caliper.")

                selected_column_BS=[bit_size] * len(well_df['DEPTH'])
            ...
            if len(selected_columns) > 0:
                ...
                for i, column in enumerate(selected_columns):
                    scatter_plot = go.Scatter(x=df_fill[column], y=df_fill['DEPTH'], mode='lines', name=column, line=dict(color=colors[i % len(colors)]))
                ...
                if st.checkbox("Finish"):
                    ...
                    # Display the DataFrame with clicked data
                    # st.subheader("Clicked Data DataFrame:")
        ...
        with st.expander("Vshale Calculation"):
            ...
            gammaray = cc1.selectbox('Gamma Ray', df_fill.columns, index=df_fill.columns.get_loc('GR') if 'GR' in df_fill.columns else 1)
    
    
    
            # gammaray=df_fill['GR']
            selected_column_GR = gammaray
            gammaray=df_fill[gammaray]
            ...
            for i in Vsh.index:  # Loop over the index
                if Vsh.loc[i] >= x and condition_good.loc[i]:
                    NPHIshc.append(df_fill[selected_column_NPHI][i])
            ...
            Nphi_correction = Vsh * NPHIshc_mean
            NPHI_shale_corrected = df_fill[selected_column_NPHI] - Vsh * NPHIshc_mean
            ...
            fig.update_layout(
                        title_text='<b>Nuetron Porosity Shale correction</b>',
                        title_font=dict(size=30, family='Arial', color='black'),
                        width=600,
                    )
            c2W.plotly_chart(fig)
        ...
        with op.expander("Lithogy Identification"):
            data = df_fill.copy() 
            column_name= st.multiselect('choose',  df_fill.columns, default=[df_fill.columns[1]])
            ...
            for i in range(len(column_name)):
                scatter_plot = go.Scatter(x=data[column_name[i]], y=data['DEPTH'], mode='lines', name=curve_name[i], line=dict(color=subplot_colors[i]))
            ...
            if aq=='Mannual Input':
                ...
                for i in range(num_zones - 1):
                    bottom_boundary = gh.number_input(f"Enter bottom boundary for Zone {i + 1}:", min_value=0.0 if i == 0 else depth_values[-1], step=0.1)
                    depth_values.append(bottom_boundary)
            
                l_df = pd.DataFrame({"Y": depth_values})
                # st.success("Added depth values to the DataFrame!")
                
                # st.write(l_df if not l_df.empty else pd.DataFrame())
            ...
            if st.button("Finish  ",key='hyn'):
                ...
                for i in range(len(column_name)):
                    scatter_plot = go.Scatter(x=data[column_name[i]], y=data['DEPTH'], mode='lines', name=column_name[i], line=dict(color=subplot_colors[i]))
                    fig.add_trace(scatter_plot, row=1, col=i + 2)
                    fig.update_yaxes(autorange='reversed', row=1, col=i + 2)
                    ...
                    for depth_value in depth_boundary:
                        fig.add_shape(
                            go.layout.Shape(
                                type='line',
                                x0=data[column_name[i]].min(),
                                x1=data[column_name[i]].max(),
                                y0=depth_value,
                                y1=depth_value,
                    ...
                    for j in range(len(depth_boundary) - 1):
                        ...
                        section_data = data[(data['DEPTH'] > lower_boundary) & (data['DEPTH'] < upper_boundary)]
        ...
        ##########################################################################  
        g_df = []
        with po.expander("Gas Bearing Zone"):
            ...
            fig.add_trace(go.Scatter(x=data[selected_column_CALI], y=data['DEPTH'], line=dict(width=1, color='blue'),showlegend=False), row=1, col=1)
    
            for depth, is_condition_met in zip(data.DEPTH,condition_bad):
                    if is_condition_met:
                        fig.add_trace(go.Scatter(
                            x=[data[selected_column_CALI].min(), data[selected_column_CALI].max()],  
                            y=[depth, depth],  
                            mode='lines',
                            line=dict(width=0.3, color='red', dash='solid'),  
                            name=f'Condition - Depth: {depth}',  
            ...
            fig.add_trace(go.Scatter(x=data[selected_column_GR], y=data['DEPTH'], line=dict(width=0.5, color='green'),name=selected_column_GR), row=1, col=2)
            
            # Add the first trace (TNPH) with X and Y swapped
            trace1 = go.Scatter(x=3 - data[selected_column_NPHI]*(15/6), y=data['DEPTH'], line=dict(width=0.8, color='blue'), name=selected_column_NPHI+'')
            fig.add_trace(trace1,row=1, col=3)
        
            
            # Add the second trace (RHOB) with X and Y swapped
            trace2 = go.Scatter(x=data[selected_column_RHOB], y=data['DEPTH'], line=dict(width=0.8, color='brown'), name=selected_column_RHOB)
            ...
            if baq == "Mannual Input":
                ...
                for i in range(num_gas_zones):
                    top_boundary = kl.number_input(f"Enter top boundary for Gas Zone {i + 1}:", min_value=0.0 if i == 0 else depth_values[-1], step=0.1)
            ...
            if st.button("Finish "):
                ...
                # Add the first trace (TNPH) with X and Y swapped
                trace1 = go.Scatter(x=3 - data[selected_column_NPHI]*(15/6), y=data['DEPTH'], line=dict(width=1.5, color='blue'), name=selected_column_NPHI)
                fig.add_trace(trace1,row=1, col=3)
                
                
                # Add the second trace (RHOB) with X and Y swapped
                trace2 = go.Scatter(x=data[selected_column_RHOB], y=data['DEPTH'], line=dict(width=1.5, color='brown'), name=selected_column_RHOB)
                fig.add_trace(trace2,row=1, col=3)
                ...
                for depth_value in g_df['Y']:
                    fig.add_shape(
                        go.layout.Shape(
                            type='line',
                            x0=data[selected_column_GR].min(),
                            x1=data[selected_column_GR].max(),
                            y0=depth_value,
                            y1=depth_value,
                            line=dict(color='magenta', width=2),
                            xref='x2',
                    ...
                    fig.add_shape(
                        go.layout.Shape(
                            type='line',
                            x0=min(data[selected_column_RHOB].min(),min(3 - data[selected_column_NPHI]*(15/6))),
                            x1=max(data[selected_column_RHOB].max(),max(3 - data[selected_column_NPHI]*(15/6))),
                            y0=depth_value,
                            y1=depth_value,
                            line=dict(color='magenta', width=2),
                            xref='x3',
                ...
                for j in range(num_gas_zones):
                        ...
                        min_value = min(data[selected_column_RHOB].min(),min(3 - data[selected_column_NPHI]*(15/6)))
                        max_value = max(data[selected_column_RHOB].max(),max(3 - data[selected_column_NPHI]*(15/6)))
                        fig.add_shape(
                            go.layout.Shape(
                                type='line',
                                x0=min_value,
                        ...
                        min_value = data[selected_column_GR].min()
                        max_value =data[selected_column_GR].max()
                        fig.add_shape(
                            go.layout.Shape(
                                type='line',
                                x0=max_value,
             ...
             ##########################################################################################################################
        #                  Density Porosity calculation and plot and total porosity calculation and plot                         #
        ##########################################################################################################################  
                        
        TPHI=[]
        #df_fill['TPHI']=np.array(len(df_fill["DEPTH"]))
        
        with op.expander("Total Porosity"):
            ...
            if abn =="DPHI Log":
                selected_column_DPHI= vcc.selectbox('Choose DPHI Log' , data.columns)
    
                DPHI = data[selected_column_DPHI]
                DPHIshc = []    
                TPHI = [] 
                x = np.percentile(Vsh, 95)
                for i in Vsh.index: 
                    if Vsh.loc[i] >= x and condition_good.loc[i]:
                        DPHIshc.append(DPHI.loc[i])
    
                DPHIshc = np.array(DPHIshc)   
                DPHIshc_mean = DPHIshc.mean()      
    
                Dphi_correction = Vsh * DPHIshc_mean
                DPHI_shale_corrected = data[selected_column_DPHI] - Vsh * DPHIshc_mean

                if gas_correction_option == 'Yes':
                    if len(g_df) % 2 == 0:
                        gas_zones_boundary = np.sort(g_df['Y'])

                        df_fill['TPHI'] = np.nan  # Initialize the column with NaN
                        ...
                        for i in range(int(len(gas_zones_boundary) / 2)):
                            ...
                            TPHI = np.zeros(len(data['DEPTH']))  # Initialize TPHI array
                            ...
                            df_fill.loc[:, 'TPHI'] = TPHI
                            unitadder(df_unit,'TPHI','V/V')
                    
                    if gas_correction_option == 'No':
                        df_fill['TPHI'] = np.nan  # Initialize the column with NaN
                        TPHI = np.zeros(len(data['DEPTH']))  # Initialize TPHI array
                        st.write('Gas correction not applied')
                        # DPHI_shale_corrected = DPHI_shale_corrected
            ...
            if abn =="Using Values":
                ...
                if bbn =="Yes":
                    ...
                    RHO_fl = float(RHO_fl)
                    data[selected_column_RHOB] = data[selected_column_RHOB].astype(float)
                    DPHI = (RHO_mat - data[selected_column_RHOB]) / (RHO_mat - RHO_fl)
                    ...
                    Dphi_correction = Vsh * DPHIshc_mean
                    DPHI_shale_corrected = DPHI -  Dphi_correction
                ...
                if bbn =="No" and not l_df.empty:
                    ...
                    depth_boundary_inner = np.array(l_df['Y'])
                    end1 = data["DEPTH"].iloc[-1]
                    end2 = data["DEPTH"].iloc[0]
                    depth_boundary =np.concatenate(([end1, end2], depth_boundary_inner))
                    depth_boundary = np.sort(depth_boundary)
                    RHOMA = []
                    ...
                    RHOMA_array = np.zeros(len(data))
                    for i in range(num_of_form):
                        condition_boundary = (data['DEPTH'] >= depth_boundary[i]) & (data['DEPTH'] <= depth_boundary[i+1])
                    ...
                    DPHI = (RHOMA_array - (data[selected_column_RHOB])) / (RHOMA_array - RHO_fl)
                    DPHIshc = []    
                    x = np.percentile(Vsh, 95)
                    for i in Vsh.index:  # Loop over the index
                        if Vsh.loc[i] >= x and condition_good.loc[i]:
                    ...
                    # Plot the lithology log as horizontal lines with colors based on the 'jet' colormap
                    for i in range(len(data)):
                        ax1.hlines(data['DEPTH'].iloc[i], 0, 1, color=plt.cm.jet(color_scale[i]), linewidth=2)
                    
                    # Customize the lithology log subplot
                    ax1.set_xlim(0, 1)
                    ax1.set_ylim(max(data['DEPTH']), min(data['DEPTH']))  # Reverse the y-axis
                    ax1.set_xlabel('Lithology Log')
                    ax1.set_ylabel('Depth (m)')
                    
                    # Plot RHOMA_array with depth in the second subplot
                    ax2.plot(RHOMA_array, data['DEPTH'], color='blue', linewidth=2)
                    ax2.set_xlim(min(RHOMA_array), max(RHOMA_array))
                    ax2.set_ylim(max(data['DEPTH']), min(data['DEPTH']))  # Reverse the y-axis
            ...
            fig.add_trace(go.Scatter(x=TPHI, y=data['DEPTH'], line=dict(width=1.5, color='green'), name='Total Porosity'), row=1, col=1)
            fig.add_trace(go.Scatter(x=df_fill[selected_column_NPHI], y=data['DEPTH'], line=dict(width=1.5, color='cyan'), name=selected_column_NPHI ), row=1, col=2)
            fig.add_trace(go.Scatter(x=df_fill['DPHI'], y=data['DEPTH'], line=dict(width=1.5, color='magenta'), name='DPHI'), row=1, col=3)
            ...
            for i in Vsh.index:  # Loop over the index
                if Vsh.loc[i] >= x and condition_good.loc[i]:
                    TPHIshc.append(TPHI[i])                  
            TPHIshc = np.array(TPHIshc)
            TPHIshc_mean = TPHIshc.mean()                    
            Tphi_correction = Vsh * TPHIshc_mean
            TPHI_shale_corrected = TPHI - Tphi_correction
            ...
            if st.checkbox('Effective Porosity calculation'):                    
                ...
                fig.add_trace(go.Scatter(x=TPHI_shale_corrected, y=data['DEPTH'], line=dict(width=1.5, color='green'), name='Effective Porosity'), row=1, col=1)
                fig.add_trace(go.Scatter(x=NPHI_shale_corrected, y=data['DEPTH'], line=dict(width=1.5, color='cyan'), name=selected_column_NPHI +'Effective Porosity' ), row=1, col=2)
                fig.add_trace(go.Scatter(x=DPHI_shale_corrected, y=data['DEPTH'], line=dict(width=1.5, color='magenta'), name='DPHI'+'Effective Porosity'), row=1, col=3)
                # Set axis titles and labels
       ...
       # ##########################################################################  
        ...
        selected_column_RESD=[]
        selected_column_RESS=[]
        with po.expander("4. Water Saturation"):
            ...
            if bbn =="Yes":
                ...
                default_column2 = 'RT' if 'RT' in df_fill.columns else ('RESD' if 'RESD' in df_fill.columns else ('AT10' if 'AT10' in df_fill.columns else df_fill.columns[1]))
                default_column3 = 'AT10' if 'AT10' in df_fill.columns else ('RESS' if 'RESS' in df_fill.columns else df_fill.columns[1])


                
                selected_column_RESD = uio.selectbox('Deep Resistivity:',data.columns, index=df_fill.columns.get_loc(default_column2))
                selected_column_RESS = oiu.selectbox('Shallow Resistivity:',data.columns, index=df_fill.columns.get_loc(default_column3))
            ...
            if bbn =="No":
                ...
                fig.add_trace(go.Scatter(x=TPHI, y=data['DEPTH'], line=dict(width=0.5, color='blue'), name='Total Porosity'), row=1, col=1)
                fig.update_xaxes(title='Total Porosity', row=1, col=1)
                fig.update_yaxes(title='Depth (m)', autorange='reversed', row=1, col=1)
                
                fig.add_trace(go.Scatter(x=data[selected_column_GR], y=data['DEPTH'], line=dict(width=0.5, color='green'), name='Gamma Ray(API)'), row=1, col=2)
                fig.update_xaxes(title='Gamma Ray(API)', row=1, col=2)
                
                fig.add_trace(go.Scatter(x=data[selected_column_RESD], y=data['DEPTH'], line=dict(width=0.5, color='blue'), name='Deep Resistivity'), row=1, col=3)
                
                fig.add_trace(go.Scatter(x=data[selected_column_RESS], y=data['DEPTH'], line=dict(width=0.5, color='red'), name='Shallow Resistivity', showlegend=False), row=1, col=3)
                fig.update_xaxes(title='Deep and Shallow Resistivity', type='log', range=[0, 2], row=1, col=3)
                
                fig.update_layout(height=1000, width=750, title_text='Logs to determine the Rw', showlegend=False)
                fig.update_layout(
                ...
                if bqq == "Mannual Input ":
                    ...
                    num_values = st.number_input("How many resistivity values do you want to input?", min_value=1, step=1)
                    resistivity_values = []
                    for i in range(num_values):
                        resistivity = st.number_input(f"Enter resistivity value {i + 1}:", step=0.01)
                        resistivity_values.append(resistivity)
                    
                    r_df = pd.DataFrame({'X': resistivity_values})
                ...
                if st.checkbox(" Finish"):
                    ...
                    # a = ss2.text_input('Input: a ',value='1.0',key='tirw')
                    # m = ss3.text_input('Input: m ',value='2.0',key='torg')
                    # n = ss4.text_input('Input: n ',value='2.0',key='trii')
                    # a= float(a)
                    # m= float(m)
                    # n= float(n)
        ...
        #             st.write(r_df)
                ...
                # Rwa_method_approvel = st.radio('Satisfied with  Rw value from Rwa method',['Yes','No'])
                # if Rwa_method_approvel== 'Yes':
                #     st.write("Rw Value:", Rw)
                # if Rwa_method_approvel== 'No':
                #     st.write('Enter Rwa Value') 
                #     Rw = st.text_input("Enter the Value of the Rw manually:", value='1')
                #     Rw = float(Rw)
            ...
            # n= float(n)
            if Rw is not None:                
                Sw_ar = [(a * Rw / (phi ** m * resd)) ** (1 / n) if phi != 0 and resd != 0 else 0 for phi, resd in zip(TPHI, data[selected_column_RESD])]
                ...
                if sim =='Archie + Simandoux Method':
                    ...
                    for i in Vsh.index:  
                        if Vsh.loc[i] >= x and condition_good.loc[i]:
                            Rshc.append(data[selected_column_RESD].loc[i])
                            Dshc.append(data['DEPTH'].loc[i])
                    ...
                    fig.add_trace(go.Scatter(x=data[selected_column_GR], y=data['DEPTH'], line=dict(width=1, color='green'),name=selected_column_GR), row=1, col=2)
                    fig.add_trace(go.Scatter(x=Vsh, y=data.DEPTH, mode='lines', name='Vsh', line=dict(color='red')), row=1, col=3)
                    
                    fig.add_trace(go.Scatter(x=data[selected_column_RESD], y=data['DEPTH'], mode='lines', name=selected_column_RESD, line=dict(color='black')), row=1, col=4)
                    ...
                    fig.update_yaxes(title_text='Depth (m)', autorange='reversed', gridwidth=0.8)
                    fig.update_layout(title='Rsh Calculation :',width=800, height=900)  # Adjust width and height as needed
                    fig.update_layout(
                    ...
                    if Rw is not None and TPHI is not None and Vsh is not None and Rsh is not None and data[selected_column_RESD] is not None:
                        # Sw_sd = (0.1/(2*TPHI**2))*(np.sqrt((Vsh/Rsh)**2 + ((4*TPHI**2) / (0.1*data[selected_column_RESD])))-(Vsh/Rsh))
                        Sw_sd = ((a * Rw) / (2 * TPHI **m)) * (np.sqrt((Vsh / Rsh)**2 + ((4 *TPHI ** m) / (a * Rw * data[selected_column_RESD]))) - (Vsh/ Rsh))
                        Sw_sd = np.array(Sw_sd)
                        Sw_sd[Sw_sd>=1]=1
                        Sw_sd[Sw_sd<=0]=0
                        col1w, col2w = st.columns(2)
                        with col2w:
                            fig, ax1 = plt.subplots(figsize=(6, 18))
                            ax1.plot(Sw_sd, data['DEPTH'], color='b', lw=1)
                            ax1.invert_yaxis()
                            ax1.tick_params(axis='x', colors='b')
                            ax1.spines['top'].set_edgecolor('b')
                            ax1.xaxis.set_ticks_position('top')
                            ax1.set_xlabel('Sw_Simandoux', color='b', fontsize=18)
                        ...
                        # Plot for Sw_ar
                        with col1w:
                            fig, ax2 = plt.subplots(figsize=(6, 18))
                            ax2.plot(Sw_ar, data['DEPTH'], color='b', lw=1)
                            ax2.invert_yaxis()
    ...
    with t3:
        ...
        with st.expander("View"):
            if uploaded_file1 is not None and (uploaded_file1.name.lower().endswith('.xlsx') or uploaded_file1.name.lower().endswith('.csv')) or dftfc:
                ...
                selected_column_top_depth = ftd.selectbox('Select top depth',f_df.columns)
                selected_column_base_depth = fbd.selectbox('Select base depth',f_df.columns)
                selected_column_formation_name = ffn.selectbox('Select Formation',f_df.columns)
                ...
                st.write("Upload Formation Tops file in Data Loading Tab")
        ...
        def plot(df_fill):
            ...
            with cola.expander("Line Plot"):
                ...
                curves = colc.multiselect('Select Curves To Plot', df_fill.columns , key="multiselect1")
                LG= cold.multiselect('Log Scale for:',curves, key="multiselect2")
                
                if len(curves) < 1:
                    st.warning('Please select at least 1 curve.')
                else:
                    curve_index = 1
                    if st.checkbox("Completed the Selection for the line plot"):
                        ...
                        for curve in curves:
                            if curve in LG:
                                ...
                                log_bool = 'linear'
                            fig.add_trace(go.Scatter(x=df_fill[curve], y=df_fill['DEPTH'], mode='lines'), row=1, col=curve_index)
            ...
            with colb.expander(" Scatter Plot"):
                colc, cold  = st.columns(2)
                curves = colc.multiselect('Select Curves To Plot', df_fill.columns, key="multiselect14")
                LG= cold.multiselect('Log Scale for:',curves, key="multiselect2o")
                
                if len(curves) < 1:
                    st.warning('Please select at least 1 curve.')
                else:
                    curve_index = 1
                    if st.checkbox("Completed the Selection for the scatter plot"):
                        fig = make_subplots(rows=1, cols=len(curves), subplot_titles=curvename(df_unit,curves), shared_yaxes=True)
                        for curve in curves:
                            if curve in LG:
                                log_bool = 'log'
                                ...
                                log_bool = 'linear'
                            fig.add_trace(go.Scatter(x=df_fill[curve], y=df_fill['DEPTH'], mode='markers', marker={'size': 4}), row=1, col=curve_index)
                            fig.update_xaxes(type=log_bool, row=1, col=curve_index)
                            curve_index += 1
                        fig.update_layout(height=1000, showlegend=False, yaxis={'title': curvename(df_unit,[deps])[0], 'autorange': 'reversed'})
                        fig.update_layout(template='plotly_dark')  # Change to 'plotly_dark' template, or use any other available template
            ...
            with cola.expander(" Histogram"):
                ...
                hist_curve = colc.selectbox('Select a Curve', df_fill.columns, index=2)
                if hist_curve is not None:
                    bin_size = colk.number_input("Bin size", value=(df_fill[hist_curve].mean())/60)
                ...
                #     log_bool = True
                # st.write(df_fill)
                if st.checkbox("Completed the selection for the Histogram"):
                    histogram = px.histogram(df_fill, x=hist_curve, log_x=False, nbins=int((df_fill[hist_curve].max() - df_fill[hist_curve].min()) / bin_size))
            ...
            with colb.expander("Cross Plot"):
                colc, cold  = st.columns(2)
                st.write('')
                xplot_x = colc.selectbox('X-Axis', df_fill.columns,index=1)
                ...
                if xplot_x_log == 'Linear':
                    ...
                    xplot_x_bool = True
                st.write('')
                xplot_y = colc.selectbox('Y-Axis', df_fill.columns,index=2)  # Change 'col1' to 'col2'
                ...
                xplot_col = cold.selectbox('Colour-Bar', df_fill.columns, index=0)
        ...
        with st.expander("Vsh Plot"):
            ...
            ax1.fill_betweenx(df_fill.DEPTH, selected_column_BS, df_fill[selected_column_CALI], facecolor='yellow')
            ...
            ax11 = ax1.twiny()
            ax11.plot(selected_column_BS, df_fill.DEPTH, lw=0.9, color='red', linestyle='dashed')
            ax11.set_xlim(5, 15)
            ...
            condition = abs(selected_column_BS - df_fill[selected_column_CALI]) > tol_val
            for depth, is_condition_met in zip(df_fill.DEPTH, condition):
                if is_condition_met:
            ...
            Dshc = []
            NPsh=[]
            condition_good = abs(df_fill[selected_column_CALI] - selected_column_BS) < tol_val
        ...
        with st.expander("Triple Combo"):
            gr_log=df_fill[selected_column_GR]
            res_log = df_fill[selected_column_RESD]
            den_log=df_fill[selected_column_RHOB]
            neu_log = df_fill[selected_column_NPHI]
            ...
            tgl=tcg.selectbox('Gamma Ray', df_fill.columns, index=df_fill.columns.get_loc('GR') if 'GR' in df_fill.columns else 1)
            gr_log=df_fill[tgl]
            
            default_column2 = 'RT' if 'RT' in df_fill.columns else ('RESD' if 'RESD' in df_fill.columns else ('AT10' if 'AT10' in df_fill.columns else df_fill.columns[1]))
            trl=tcr.selectbox('Resistivity:',data.columns, index=df_fill.columns.get_loc(default_column2))
            res_log = df_fill[trl]

            tdl=tcd.selectbox('RHOB', data.columns, index=data.columns.get_loc('RHOB') if 'RHOB' in data.columns else 1)
            den_log=df_fill[tdl]

            tnl=tcn.selectbox('NPHI', df_fill.columns, index=df_fill.columns.get_loc(default_column))
            neu_log = df_fill[tnl]
            ...
            def create_triple_combo_plot(df_fill, gr_log, res_log, den_log, neu_log):
                ...
                plot_h, plot_w, title_size, title_height, line_width = 17, 12, 12, 1.0, 1

                # Define track parameters
                gr_params = {'color': 'teal', 'trackname': curvename(df_unit,[tgl])[0], 'left': 0, 'right': 200, 'cutoff': 60, 'base': 0,
                             'sand_color': 'gold', 'shale_color': 'lightslategray', 'div': 5}

                res_params = {'color': 'darkorange', 'trackname': curvename(df_unit,[trl])[0], 'left': 0.2, 'right': 20000, 'cutoff': 100, 'shading_color': 'navajowhite'}

                den_params = {'color': 'brown', 'trackname': curvename(df_unit,[tdl])[0], 'left': 1.95, 'right': 2.95}

                neu_params = {'color': 'darkslategray', 'trackname': curvename(df_unit,[tnl])[0], 'mean': np.nanmean(neu_log),
                ...
                ax1 = axes[0]
                ax1.plot(gr_log, "DEPTH", data=df_fill, color=gr_params['color'], lw=line_width)
                ax1.set_xlabel(gr_params['trackname'])
                ax1.set_xlim(gr_params['left'], gr_params['right'])
                ax1.set_ylim(bot_depth, top_depth)
                ax1.fill_betweenx(df_fill['DEPTH'], gr_params['base'], gr_log, where=(gr_params['cutoff'] >= gr_log),
                ...
                ax2 = axes[1]
                ax2.plot(den_log, "DEPTH", data=df_fill, color=den_params['color'], lw=line_width)
                ...
                ax4 = ax2.twiny()
                ax4.plot(neu_log, "DEPTH", data=df_fill, color=neu_params['color'], lw=line_width)
                ...
                ax3 = axes[2]
                ax3.plot(res_log, "DEPTH", data=df_fill, color=res_params['color'], lw=line_width)
                ax3.set_xlabel(res_params['trackname'])
                ax3.set_xlim(res_params['left'], res_params['right'])
                ax3.set_ylim(bot_depth, top_depth)
                ax3.semilogx()
                ax3.fill_betweenx(df_fill['DEPTH'], res_params['cutoff'], res_log, where=(res_log >= res_params['cutoff']),
                ...
                st.download_button(
                  ...
                  file_name="triple_combo_plot.pdf",
                )

            create_triple_combo_plot(df_fill, gr_log, res_log, den_log, neu_log)





        with st.expander("NTG"):
            st.subheader('NTG')
            st.title('')
            st.write('Conditions for NTG')
            ncl,nlc,cln= st.columns(3)
            nvsh = ncl.number_input('Vsh', value=0.3)    
            # Sw = nlc.radio("Sw",['Sw_ar','Sw_sd'])
            if sim =='Archie + Simandoux Method':
                Sww = nlc.radio("Sw",['Archie','Simadaux'])
                if Sww=='Archie':
                    Sw=Sw_ar
                if Sww=='Simadaux':
                    Sw=Sw_sd
                ...
                Sw=Sw_ar
            nswsd = nlc.number_input('',value=0.6)
            ntphi = cln.number_input('TPHI', value=0.05)            
        ...
        with st.expander("View"):
            if uploaded_file2 is not None and (uploaded_file2.name.lower().endswith('.xlsx') or uploaded_file2.name.lower().endswith('.csv')) or dftcc:
                ...
                x = rty.selectbox('Select Depth', c_df.columns)
                y = yty.selectbox('Select Porosity', c_df.columns)
                z = tyr.selectbox('Permeability', c_df.columns)
