...
try:
    ...
    import lasio  # Import the lasio library
...
class FileUpload(object):
    ...
    def __init__(self):
        self.fileTypes = ["csv", "png", "jpg", "las"]  # Add "las" to the list of accepted file types
    ...
    def run(self):
        ...
        show_file = st.empty()
        if not file:
            show_file.info("Please upload a file of type: " + ", ".join(["csv", "png", "jpg", "las"]))
            return
        content = file.getvalue()
        if isinstance(file, BytesIO):
            show_file.image(file)
        else:
            if file.type == "application/octet-stream" and file.name.endswith(".las"):
                # If it's an LAS file, you can use lasio to read it
                las = lasio.read(file)
                # You can access the data using las.sections and other attributes
                st.write(las.sections)
            else:
